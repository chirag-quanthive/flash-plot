...
class Axes:
    ...
    def _render(self, bounds: Rect) -> SubplotScene:
        ...
        if _is_pie_only:
            ...
            # Right y-axis panel (72px) + x-labels (18px) + gap (24px) + bottom bar (24px)
            pa = compute_layout(bounds.w, bounds.h, padding=Padding(top=4, right=72, bottom=78, left=10), inset=8)
        else:
