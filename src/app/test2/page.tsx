...
const EXAMPLES: Record<string, string> = {
  ...
  histogram: JSON.stringify({
    ...
    subtitle: "Daily returns (20 bins — scroll to see all)",
    series: [
  ...
  surface: JSON.stringify({
    ...
    surface: {
      z: [
        ...
        [0.2, 0.18, 0.17, 0.16, 0.15, 0.15, 0.15, 0.14],
        [0.19, 0.18, 0.17, 0.16, 0.15, 0.15, 0.14, 0.14],
      ],
      wireframe: true,
    },
  }, null, 2),
...
// ── Pie color defaults (same as theme) ──────────────────────────────────
...
export default function Test2Page() {
  const [specText, setSpecText] = useState(EXAMPLES.line);
  const [result, setResult] = useState<RenderResult | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  const [mode, setMode] = useState<RenderMode>("local");
  const [activeExample, setActiveExample] = useState("line");
  ...
  const handleRender = useCallback(async () => {
    ...
    try {
      ...
      // Apply chart settings; strip title/subtitle (rendered by ChartHeader instead)
      const spec = {
        ...rawSpec,
        title: undefined,
        subtitle: undefined,
    ...
    <div className="min-h-screen bg-[#121212] text-white" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="border-b border-[#2a2a2a] px-6 py-4">
        <h1 className="text-[22px] font-normal" style={{ fontFamily: "var(--font-eb-garamond), 'EB Garamond', Georgia, serif" }}>
          Flash Plot — MCP Test Playground
        </h1>
        <p className="text-[#707070] text-[13px] mt-1">
          Enter a ChartSpec JSON and render it locally or via the /api/chart/render endpoint
        </p>
      </div>
      ...
      <div className="flex h-[calc(100vh-73px)]">
        ...
        <div className="flex-1 flex flex-col overflow-auto">
          ...
          <div className="flex-1 flex items-center justify-center p-8">
            ...
            {!result && !error && (
              <div className="text-[#333] text-[14px]">
                Edit the ChartSpec and click Render
              </div>
            )}
          ...
          {/* Scene JSON preview (collapsible) */}
          {result && (
            <ScenePreview result={result} />
          )}
        </div>
      </div>
    </div>
